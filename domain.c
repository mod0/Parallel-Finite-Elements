...
domain* build_cartesian_domain(grid* cartesian_grid, int subdomain_count_x, int subdomain_count_y, vertex_numbering_dir direction)
{
  ...
  cartesian_domain->subdomain_count_y = subdomain_count_y;
...
{
  int i, j;
  int subdomain_count_x, subdomain_count_y;
  subdomain** all_subdomains;
  subdomain* cartesian_subdomain;

  subdomain_count_x = cartesian_domain->subdomain_count_x;
  subdomain_count_y = cartesian_domain->subdomain_count_y;

  all_subdomains = (subdomain**) calloc(subdomain_count_x*subdomain_count_y, sizeof(subdomain*));

  for(i = 0; i < subdomain_count_y; i++)
  {
    for(j = 0; j < subdomain_count_x; j++)
    {
      ...
      cartesian_subdomain->top;                                                 // Pointer to top subdomain
      cartesian_subdomain->bottom;                                              // Pointer to bottom subdomain
      ...
      cartesian_subdomain->ghost_subdomain_top;                                 // Ghost cells into which neighboring threads will write info
      cartesian_subdomain->ghost_subdomain_bottom;                              // Ghost cells into which neighboring threads will write info
